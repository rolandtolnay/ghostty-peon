...
| `hooks/tabtitle-hook.py` | UserPromptSubmit: sets 🌀 working emoji and generates a slug via Ollama; explicit `scope`/`prep` skills add a stateless prefix |
...
# Explicit scope/prep title prefixes
grep "skill prefix ->" /tmp/pi-tab-hooks.log
...
If no `skill prefix ->` line appears, inspect the prompt mapping and confirm the skill envelope reached `before_agent_start`. Other skills and ordinary prompts intentionally use unprefixed opportunistic titles.
