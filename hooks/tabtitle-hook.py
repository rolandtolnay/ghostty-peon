...
def generate_slug(
    ...
    system = (
        "Generate a lowercase hyphenated slug (3-5 words) for a terminal tab title. "
        "Start with an action verb (fix, add, refactor, debug, setup, update, review).\n"
        "Short commands or ticket IDs: slugify directly ('/work-ticket MIN-163' → 'work-ticket-min-163')\n\n"
        ...
        "  title=fix-modal-rendering, msg='Test that in the browser' → KEEP\n"
        "  title=fix-modal-rendering, msg='Do you have auth state already?' → KEEP\n"
        "  title=add-csv-export, msg='I disagree, try X instead' → KEEP\n\n"
        "RENAME — user moved to a different feature:\n"
