...
Unit tests use `tests/helpers.py` as a top-level import, so run targeted unittest modules with `PYTHONPATH=tests`:

```sh
PYTHONPATH=tests python3 -m unittest tests.test_session_end_pi tests.test_session_sound_pi
python3 -m unittest discover -s tests
```

Plain `python3 -m unittest` from the repo root discovers 0 tests; use explicit discovery instead.
